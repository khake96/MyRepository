...
async function oneEmployeeHistory(employeeId) {
    try {
      resp = await fetch(url+'employeeHistory',
      {
      method:'POST',
      headers: { 
        'Content-Type':  
  ...
  buildHistoryTable(result);
    ...
    document.getElementById("generalErrorModalParagraph").innerHTML = "Could not execute request. Please try again."
...
async function getTable(urlSpecific) {
  ...
  if(resp.status === 200){
    ...
    document.getElementById("generalErrorModalParagraph").innerHTML = "Whoops. We lost your credentials. PLease log out and try again."
    $("#genericErrorModal").modal("show");
  }
};
...
async function newRequestFunc() {
  ...
  if(resp.status === 200){
    ...
    let ts_ms = newReimbResponse.requestDate;
