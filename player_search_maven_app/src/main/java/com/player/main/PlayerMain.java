...
//			}
			...
			case 1:
				...
				break;
			case 2:	{
			System.out.println("Please Enter Player name to get Player Details");
			try {
				String name = scanner.nextLine();
				//Code Here for SERVICE LAYER
				Player player= playerSearchService.getPlayerByName(name);
				if(player!=null) {
					System.out.println("We found a player by name = "+name+" Details are : ");
					System.out.println(player);
				}
			} catch (NumberFormatException e) {
				System.out.println("Could not find player by that name.. Kindly Re-enter");
			} catch (BusinessException e) {
				System.out.println(e.getMessage());
			}
			break;
			...
			case 3: {				
				System.out.println("Please enter age to get the Players with that age ");
				int age=Integer.parseInt(scanner.nextLine());
			try {
				List<Player> playersListAge=playerSearchService.getPlayersByAge(age);
				if(playersListAge!=null && playersListAge.size()>0) {
					System.out.println("We Found "+playersListAge.size()+" no of player/s in the DB..with age "+age+" Details are");
					for(Player p:playersListAge) {
			...
			case 5:
				System.out.println("Please Enter Player contact to get Player Details");
				try {
					Long contact = Long.parseLong(scanner.nextLine());
					//Code Here for SERVICE LAYER
					Player player=playerSearchService.getPlayerByContactNumber(contact);
					if(player!=null) {
						System.out.println("We found a player by contact= "+contact+" Details are : ");
						System.out.println(player);
					}
				} catch (NumberFormatException e) {
					System.out.println("Contact should be number only.. Kindly Re-enter");
				} catch (BusinessException e) {
					System.out.println(e.getMessage());
				}
				break;
			case 6:
				System.out.println("Please enter Team Name to get the Players on that Team ");
				...
				try {
					...
					if(playersListTeam!=null && playersListTeam.size()>0) {
						System.out.println("We Found "+playersListTeam.size()+" no of player/s in the DB..on the team: "+teamName+" Details are");
